...
// convenience function to compactly check assertions on a pointer to a key expression with a musig
static void check_key_expr_musig(const policy_node_keyexpr_t *ptr,
                                 int n_musig_keys,
                                 const uint16_t *key_indices,
                                 uint32_t num_first,
                                 uint32_t num_second) {
...
static void test_parse_policy_tr_musig_scriptpath(void **state) {
    ...
    res = parse_policy("tr(@1/**,pk(musig(@2,@0,@3)/**))", out, sizeof(out));
...
static void test_failures(void **state) {
    ...
    // invalid usages of musig expressions
    assert_true(0 > parse_policy("tr(musig(@0,@1))", out, sizeof(out)));  // missing derivations
    assert_true(0 > parse_policy("tr(musig()/**)", out, sizeof(out)));    // empty musig
    assert_true(0 > parse_policy("tr(musig(@0)/**)", out, sizeof(out)));  // needs at least two keys
