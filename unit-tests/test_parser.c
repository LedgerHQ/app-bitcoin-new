...
const parsing_step_t parse_ABC_steps[] = {
    (parsing_step_t)parse_A,
...
const parsing_step_t parse_ABC_error_steps[] = {
    (parsing_step_t)parse_A,
...
static void test_parser_stream_ends(void **state) {
    ...
    assert_int_equal(stream_buf.offset, 0); // since read failed, the offset should not have changed
...
static void test_parser_continue_partial(void **state) {
    ...
    uint8_t store[32] = {
        0xb0, 0xb1     // leftover from before
    };
    uint8_t stream[32] = {
    ...
    assert_int_equal(stream_buf.offset, 6 + 1); // since read failed, the offset should not have changed
