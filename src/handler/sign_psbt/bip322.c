...
bool bip322_detect(dispatcher_context_t *dc, sign_psbt_state_t *st) {
    ...
    // Check whether the field is present at all. A client that hides (or never sets) the field
    // gets the regular transaction review instead: the to_sign transaction is then shown as a
    // zero-value transaction with a bare OP_RETURN output, exactly as before this feature
    // existed. What this flow guarantees is that the message review UI is only ever shown
    // after bip322_validate() has bound the displayed message to the produced signature.
    uint8_t key_hash[32];
    merkle_compute_element_hash(key, sizeof(key), key_hash);
    if (call_get_merkle_leaf_index(dc, st->global_map.size, st->global_map.keys_root, key_hash) <
        0) {
        st->bip322.is_message_signing = false;
        return true;
    }

    // The field is present: stream the message, computing its hashes and printability.
    ...
    int message_length = call_stream_merkleized_map_value(dc,
                                                          ...
                                                          &hash_state);
    if (message_length < 0) {
        // the key was found above, so this is a protocol error, not an absent field
