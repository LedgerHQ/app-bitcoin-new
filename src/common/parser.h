...
/**
 * A parsing step gets a pointer to the parser's state, and an array of two pointers to buffers. The concatenation o
 * the two buffer is the (possibly incomplete) data to be parsed.
 * The parsing step returns -1 in case of parsing error (e.g.: invalid value was parsed); 1 if the parsing step is
 * completed successfully, 0 if more data is expected.
 * Any remaining data in the concatenation of the two buffers' remaining content must be passed in the next call to the
 * parser, that will continue from the same parsing step.
 */
typedef int (*parsing_step_t)(void *, buffer_t *[2]);


// Convenience functions to handle reading from the concatenation of two buffers.
// All these functions are analogouse to the corresponding buffer_read_X functions, but they exhaust the first buffer
// before reading from the second buffer.
...
/**
 * Moves the concatenation of all the remaining bytes in the two buffers into the memory pointed by the first buffer,
 * as long as the number of remaining bytes is at most max_byte.
 * The offset of the first buffer is set to 0, and the new size reflects the total size 
 *
