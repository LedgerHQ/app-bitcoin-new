...
/*
  ...
  - internal element hashes for a note with left child hashing to l_hash and right child hashing to r_hash is:
    SHA256(0x01 ||| l_hash ||| r_hash)

  This ensures that no two trees with the same root hash can be computed.

  This implementation has 128 bits of collision-resistance security if the attacker can choose the value of leaves.
  If the attacker cannot control inserted leaves, then finding a collision is as hard as chosen-preimage for SHA256.
*/
...
/**
 * Convenience method to compute the hash of an element for the Merkle tree, which is the SHA256 hash of the
 ...
 * The result is the output of SHA256 on the concatenation of the byte 0x01, the left child hash, and the right
 * child hash.
 *
 * @param[in] left
 *   Pointer to the input buffer.
